...
utils._cssPathStep = function(node, optimized, isTargetNode)
{
    ...
    if (optimized) {
        ...
        if (nodeNameLower === "body" || nodeNameLower === "head" || nodeNameLower === "html")
            return new utils.DOMNodePathStep(node.nodeNameInCorrectCase(), true);
    }
    var nodeName = node.nodeNameInCorrectCase();

    if (id)
        return new utils.DOMNodePathStep(nodeName + idSelector(id), true);
    var parent = node.parentNode;
    if (!parent || parent.nodeType === Node.DOCUMENT_NODE)
        return new utils.DOMNodePathStep(nodeName, true);
...
document.body.addEventListener('mouseover', function(evt){
    [].forEach.call(document.querySelectorAll('.overed'), function(el) {
        el.classList.remove('overed');
    });
    evt.target.classList.add('overed');
    console.log(utils.cssPath(evt.target));
});